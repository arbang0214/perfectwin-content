...
이 시스템은 매일/매주/매월 GA4·Search Console·inblog 데이터를 수집하고, Claude로 리포트를 생성해 Slack(및 이메일)으로 발송합니다. **GitHub Actions가 전부 자동으로 돌립니다 — 별도 서버 불필요.**
...
4. 실패 시 로그의 어느 수집 단계(GA4/GSC/inblog)에서 멈췄는지 보면 어떤 시크릿이 문제인지 바로 드러남.
...
- [ ] ⚠️ 개인 종속 키(Anthropic / Slack / inblog / Gmail) 새 명의로 교체
