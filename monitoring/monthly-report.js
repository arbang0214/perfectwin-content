...
## 이번 달 Demo Funnel 집계
${thisData.annual.demoFunnel ? JSON.stringify(thisData.annual.demoFunnel, null, 2) : "데모 퍼널 데이터 없음"}
...
## 전월 Demo Funnel 집계
${prevData?.annual.demoFunnel ? JSON.stringify(prevData.annual.demoFunnel, null, 2) : "전월 데이터 없음"}
...
## 2개월 전 Demo Funnel 집계
${prev2Data?.annual.demoFunnel ? JSON.stringify(prev2Data.annual.demoFunnel.totals, null, 2) : "데이터 없음"}

## 3개월 전 Demo Funnel 집계
${prev3Data?.annual.demoFunnel ? JSON.stringify(prev3Data.annual.demoFunnel.totals, null, 2) : "데이터 없음"}

이 데이터를 기반으로 홈페이지 월간 인사이트 리포트를 작성해줘.

### 리포트 구조
#### 1. 월간 핵심 요약
핵심 지표 테이블 (전월 대비). 경영진이 30초에 파악할 수 있는 3줄 요약. **데모 신청 건수(demoFunnel.totals.submissions) 핵심 지표 포함**.
#### 2. 트래픽 추이 (전월/3개월 대비)
#### 3. 참여도 분석
#### 4. 유입 채널 분석 (채널별 비중 변화, Organic 성장 여부)
#### 5. Top 페이지 + 전환 퍼널 분석
demoFunnel 데이터 활용: 전체 트래픽 → 데모 페이지 도달(intent) → submit 완료의 3단 퍼널.
#### 5-1. 데모 신청 어트리뷰션 (월간 누적)
demoFunnel.totals.submissions ≥ 1이면 작성:
- 월 누적 submit 건수, 전환율, 전월/3개월 대비 추이.
- **submit.byLandingPage Top 10**: 데모로 가장 많이 보낸 페이지·블로그 랭킹 — 경영진 보고용.
- submit.bySourceMedium: 채널 분포.
- first-touch (byFirstUserSource) vs last-touch (bySourceMedium) 비교 — 콘텐츠 인지 → 전환 경로 차이.
#### 6. 기기/국가 분포
#### 7. 3개월 트렌드 테이블 (방향 ↑↓→)
데모 신청 건수도 트렌드 행에 포함.
#### 8. 인사이트 (6~8개)
비즈니스 임팩트 순. 각 인사이트는 아래 구조로 **충분히 자세하게** 서술:
- **현상**: 구체적 수치 + 전월/3개월 대비 변화량·변화율. 트렌드 방향 명시.
- **왜 이런 일이 생겼나**: 원인 분석. 어떤 채널/페이지/기기/국가에서 발생했는지 드릴다운. 교차 지표 근거 명시.
- **비즈니스 임팩트**: 전환(데모 요청)이나 브랜드 인지에 어떤 영향인지, 이 추세가 계속되면 어떻게 되는지 설명.
- **→ 액션**: "어떤 페이지의 어떤 요소를 어떻게 수정" 수준의 구체적 지시. 우선순위/난이도 표시.
...
## 이번 달 Demo Funnel 집계 (블로그 → 데모 어트리뷰션)
${thisData.annual.demoFunnel ? JSON.stringify(thisData.annual.demoFunnel, null, 2) : "데모 퍼널 데이터 없음"}
...
### 리포트 구조
#### 1. 월간 핵심 요약
영문/한글 블로그 + GSC 핵심 지표 (전월 대비). 3줄 핵심 문장. **데모 신청 기여 블로그 수도 포함**.
#### 2. 블로그 트래픽 (영문/한글 각각, 전월/3개월 대비)
#### 3. 오가닉 유입 + SEO 효과 분석
#### 4. 유입 소스 분석
#### 5. 검색 성과 (노출/클릭/CTR/순위, 핵심 키워드 Top 15, 핵심 페이지 Top 15)
"노출 많고 클릭 0" → 메타 리라이트 대상. "순위 8~15" → 첫 페이지 진입 기회.
#### 5-1. 데모 신청 기여 블로그 (월간 누적 — 콘텐츠 전략 핵심)
demoFunnel.totals.submissions ≥ 1일 때만 작성.
- submit.byLandingPage에서 블로그 path만 추출하여 Top 10 표.
- 각 블로그별: 누적 submit 세션, 같은 달 inblog 방문수, "방문 → submit" 비율.
- **"어떤 주제·키워드·각도가 데모로 잘 이어졌는가" 패턴 분석** — 다음 달 콘텐츠 우선순위 결정에 사용되는 결정적 신호.
- 전월/3개월 대비 어느 주제가 새로 효과를 내고 있는지 추세 언급.
#### 6. 3개월 트렌드 테이블 (방향 ↑↓→)
#### 7. 인사이트 (6~8개)
비즈니스 임팩트 순. 각 인사이트는 아래 구조로 **충분히 자세하게** 서술:
- **현상**: 구체적 수치 + 전월/3개월 대비 변화량·변화율. 트렌드 방향 명시.
- **왜 이런 일이 생겼나**: 원인 분석. 어떤 키워드/포스트/소스에서 발생했는지 드릴다운. 교차 지표(inblog ↔ GSC) 근거 명시.
- **비즈니스 임팩트**: SEO 성장이나 리드 확보에 어떤 영향인지, 이 추세가 계속되면 어떻게 되는지 설명.
- **→ 액션**: "어떤 포스트의 어떤 요소를 어떻게 수정" 수준의 구체적 지시. 우선순위/난이도 표시.
